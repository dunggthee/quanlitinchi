...
include_once("../publish/connect.php");
  ...
  $kq=mysqli_query($conn,$sql);
      ...
      while ($row=mysqli_fetch_array($kq)) {
        ...
        $dsgiangvien = $row['giangvien'];
        $dssotinchi = $row['sotinchi'];
        $dshocphi = $row['hocphi'];


        if (mysqli_num_rows(mysqli_query($conn,"SELECT mamonhoc FROM dsdangky WHERE mamonhoc='$mamonhoc'")) > 0)
       {
           echo "Bạn đã đăng kí môn này rồi. <a href='javascript: history.go(-1)'>Trở lại</a>";
           exit;
       } 	
...
$sql = "SELECT SUM(dssotinchi) FROM dsdangky Where masinhvien='$masinhvien'";
    ...
    if ($row['SUM(dssotinchi)']>15) {
      ...
      echo "Đăng kí thành công. <a href='http://localhost/hi/view/dangky.php'>Tiếp tục</a>";
    }
  }
}
