...
.container-login101::before {
 content: "";
  ...
  top: 0;
  left: 0;
  background: -webkit-linear-gradient(top, #f29df2, #ff56aa);
}
		.wrap-login101 {
    width: 100%;
    border-radius: 10px;
    ...
    background: #326da8;
    background: -webkit-linear-gradient(top, #b8d5f2, #79b8f7);
    position: relative;
    top: 30px;
}
td{
...
<div class="limiter">
		<div class="container-login101" >
          ...
          if (mysqli_num_rows($kq) > 0) {
            {while ($row= mysqli_fetch_assoc($kq)) {
                  # code...
                  echo "<img height='150px' width='150px' style='border-radius: 10%;'' src='".$row['avatar']."'>";
