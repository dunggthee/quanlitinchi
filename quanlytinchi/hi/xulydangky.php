...
// Nếu không phải là sự kiện đăng ký thì không xử lý
    ...
    if (!$username || !$password || !$hoten || !$lop || !$masinhvien )
    {
    ...
    if (mysqli_num_rows(mysqli_query($conn,"SELECT masinhvien FROM sinhvien WHERE masinhvien='$masinhvien'")) > 0)
    {
        echo "Mã sinh viên này đã có người dùng. Vui lòng nhập mã sinh viên khác. <a href='javascript: history.go(-1)'>Trở lại</a>";
        exit;
    }
	...
	if ($kq) 
	{
		echo "Chúc mừng bạn đã đăng ký tài khoản thành công. <a href='http://localhost/hi/login.php'>
							Đăng nhập ngay!
						</a>";
	}
    else
